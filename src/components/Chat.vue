...
export default {
  ...
  sockets: {
    ...
    message (data) {
      console.log(data) // this function gets triggered once a socket event of `message` is received
  ...
  methods: {
    sendMessage () {
      ...
      this.$socket.emit('message', this.nom + ':' + this.message) // send the content of the message bar to the server
